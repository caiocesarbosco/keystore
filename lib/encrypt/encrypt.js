...
async function deriveKey(password) {
  ...
  let passwordKey = await crypto.importKey("raw", encoder.encode(password), derive_key_algorithm, false, config.KEY_USAGE.DERIVE);
  ...
  var signDerivedKey = await crypto.deriveKey(kdfParams, passwordKey, signAlgorithm, true, config.KEY_USAGE.SIGN_VERIFY);
  var encryptDerivedKey = await crypto.deriveKey(kdfParams, passwordKey, encriptAlgorithm, true, config.KEY_USAGE.ENCRYPT_DECRYPT);
...
async function encryptsKeyPairFile(data, key, password) {
  ...
  var cipher = await crypto.encrypt({name: encrypt_algorithm, counter: iv, length: 32}, key, data);
  return cipher;
}
...
async function decryptsKeyPairFile(data, key) {
  ...
  var iv = utils.asciiToUint8Array("AnyRandomData012");
  var plainText = await crypto.decrypt({name: encrypt_algorithm, counter: iv, length: 32}, key, data);
  return plainText;
}
